...
public class GmcCMD extends CustomCMD {
    public GmcCMD(SpigotMain plugin) {
        super(plugin);
        setName("gmc");
        addAliases("gm1", "gmcreative");
        setPermission("minecraft.command.gamemode");
        setDescription("Cambia tu modo de juego a creativo.");
        setTabComplete(true);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 && sender instanceof Player) {
            ...
            String msg_mode = getPlugin().getUtils().getLangMSG(player, "commands.gamemode.modes.CREATIVE");
        ...
        if (args.length == 1){
            ...
            if (target != null) {
                ...
                msg_mode = getPlugin().getUtils().getLangMSG(target, "commands.gamemode.modes.CREATIVE");
                msg = msg.replace("{mode}", msg_mode);
                ...
                msg_mode = getPlugin().getUtils().getLangMSG(sender, "commands.gamemode.modes.CREATIVE");
                msg = msg.replace("{mode}", msg_mode);
            ...
            if (args[0].equalsIgnoreCase("all")){
                for (Player player : getPlugin().getServer().getOnlinePlayers()){
                    ...
                    msg_mode = getPlugin().getUtils().getLangMSG(player, "commands.gamemode.modes.CREATIVE");
                    msg = msg.replace("{mode}", msg_mode);
                    getPlugin().getUtils().sendMessage(player, msg);
                }
