...
public class GmaCMD extends CustomCMD {
    public GmaCMD(SpigotMain plugin) {
        super(plugin);
        setName("gma");
        addAliases("gm2", "gmadventure");
        setPermission("minecraft.command.gamemode");
        setDescription("Cambia tu modo de juego a aventura.");
        setTabComplete(true);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 && sender instanceof Player) {
            ...
            String msg_mode = getPlugin().getUtils().getLangMSG(player, "commands.gamemode.modes.ADVENTURE");
        ...
        if (args.length == 1){
            ...
            if (target != null) {
                ...
                msg_mode = getPlugin().getUtils().getLangMSG(target, "commands.gamemode.modes.ADVENTURE");
                msg = msg.replace("{mode}", msg_mode);
                ...
                msg_mode = getPlugin().getUtils().getLangMSG(sender, "commands.gamemode.modes.ADVENTURE");
                msg = msg.replace("{mode}", msg_mode);
            ...
            if (args[0].equalsIgnoreCase("all")){
                for (Player player : getPlugin().getServer().getOnlinePlayers()){
                    ...
                    msg_mode = getPlugin().getUtils().getLangMSG(player, "commands.gamemode.modes.ADVENTURE");
                    msg = msg.replace("{mode}", msg_mode);
                    getPlugin().getUtils().sendMessage(player, msg);
                }
